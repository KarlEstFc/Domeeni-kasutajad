KIRJELDUS
---------
Skript loob ja uuendab Active Directory kasutajaid
nimekirjafaili põhjal.
...
2. Arvutis/serveris peab olema Active Directory
   PowerShell moodul ning ligipääs AD domeenile.
...
Asukoht võib sisaldada komasid. Skript kasutab viimast
koma asukoha ja ameti eraldamiseks.
 
5. Kui PowerShell blokeerib skripti käivitamise:
 
   Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
 
See muudatus kehtib ainult praeguses PowerShelli protsessis.
...
Teise faili kasutamine:
   ...
   & ".\AD_Users V2 (Universal).ps1" `
       -CsvPath "C:\AD\users.csv"
 
OU nimede muutmine:
 
   & ".\AD_Users V2 (Universal).ps1" `
       -UsersOUName "Employees" `
       -GroupsOUName "JobGroups"
 
Teise algparooli kasutamine:
 
   & ".\AD_Users V2 (Universal).ps1" `
       -DefaultPassword "MinuParool123!"
...
Kui sama kasutajanimi on juba kasutusel teise inimese poolt,
lisatakse lõppu number.
...
Grupi sAMAccountName genereeritakse eraldi ja sellele
lisatakse lühike hash, et vältida nimekonflikte.
 
============================================================
TÄHTIS
============================================================
 
- Domeeni nime ei pea käsitsi määrama.
- Skript kasutab Get-ADDomain käsku.
- Olemasolevaid OU-sid ei looda uuesti.
- Olemasolevaid gruppe ei looda uuesti.
- Olemasolevate kasutajate Title ja Office uuendatakse.
- Skripti võib käivitada mitu korda.
- Skripti käivitaval kasutajal peavad olema vajalikud
  Active Directory õigused.
...
try {
    ...
    Write-Host "[FATAL] Active Directory module could not be loaded." `
        -ForegroundColor Red
...
try {
    ...
    Write-Host "[FATAL] Active Directory domain could not be detected." `
        -ForegroundColor Red
    Write-Host $_.Exception.Message
    exit 1
}
...
function Convert-ToUsername {
    ...
    # Special characters that Unicode normalization
    # does not always convert as expected.
    # Explicit [string] forces Replace(string, string).
    ...
    # Normalize characters with diacritics:
    # ä -> a
    # ö -> o
    # ü -> u
    # õ -> o
    ...
    # Remove dots from beginning/end
    $Text = $Text.Trim('.')
...
function Ensure-OU {
    ...
    try {
        ...
        Write-Host "[ERROR OU] $Name : $($_.Exception.Message)" `
            -ForegroundColor Red
...
function Get-DepartmentOU {
    ...
    switch -Regex ($Job) {
        ...
        '(?i)Project Manager|Product Manager|Manager' {
            return "Management"
        }
        ...
        '(?i)Financial|Accountant|Business Analyst' {
            return "Finance"
        }
 
        '(?i)Mechanical Engineer|Architect|Research Scientist' {
            return "Engineering"
        }
 
        '(?i)IT|Software|Web|Database|Data Analyst|Developer' {
            return "IT"
        }
...
function Get-GroupSamAccountName {
    ...
    # Create short deterministic hash
    ...
    # Keep sAMAccountName short and unique
    $MaxJobLength = 11
 
    if ($SafeJob.Length -gt $MaxJobLength) {
        $SafeJob = $SafeJob.Substring(
            0,
            $MaxJobLength
        )
    }
...
# Format:
#
# Firstname Lastname - Location, Job
#
# Last comma separates Location and Job.
#
# Example:
#
# Tina King - Washington, D.C., Journalist
...
foreach ($Job in $Jobs) {
    ...
    if ($SAMCollision) {
        ...
        # Extremely unlikely, but handle it safely
    ...
    try {
        ...
        Write-Host "[ERROR GROUP] $GroupName : $($_.Exception.Message)" `
            -ForegroundColor Red
    }
}
...
foreach ($User in $Users) {
    ...
    try {
        ...
        $BaseUsername = Convert-ToUsername (
        ...
        # AD sAMAccountName should be kept <= 20 chars
        ...
        if (-not $ExistingUser) {
            ...
            $ADUser = Get-ADUser `
                -Identity $Username `
                -Properties Title,Office
 
 
            $CreatedUsers++
 
 
            Write-Host "[CREATED USER] $($User.FullName) [$Username]" `
                -ForegroundColor Green
        }
        ...
        $AlreadyMember = Get-ADGroupMember `
            ...
            -ErrorAction Stop |
            Where-Object {
                $_.DistinguishedName -eq $ADUser.DistinguishedName
            }
        ...
        Write-Host "[ERROR] $($User.FullName): $($_.Exception.Message)" `
            -ForegroundColor Red
    }
}
